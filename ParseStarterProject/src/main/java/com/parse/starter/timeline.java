...
public class timeline extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        ParseQuery<ParseUser> getfollowing = ParseUser.getQuery();
        getfollowing.whereEqualTo("username", ParseUser.getCurrentUser().getUsername());
        getfollowing.findInBackground(new FindCallback<ParseUser>() {
            @Override
            public void done(List<ParseUser> objects, ParseException e) {
                if (e == null) {
                    if (objects.size() > 0) {
                        for (ParseObject user : objects) {
                            if (user.getList("following") != null) {
                                following.addAll(user.<String>getList("following"));
                            }
                        }
                    ...
                    query2.whereContainedIn("username", following);
                            ...
                            if (e == null) {
                                {
                                    ...
                                    query.whereContainedIn("createdby", following);
                                    query.findInBackground(new FindCallback<ParseObject>() {
                                        ...
                                        public void done(List<ParseObject> objects, ParseException e) {
                                            if (e == null) {
                                                ...
                                                query3.whereContainedIn("username", following);
            ...
            getinfo.findInBackground(new FindCallback<ParseUser>() {
                ...
                public void done(List<ParseUser> objects, ParseException e) {
                    if (e == null) {
                        ...
                        }else if (Contents.get(finalI).getType().matches("canceledticket")) {
                            whathappen.setText("Cancelled ticket ");
                        }
                                                                ...
                                                                if (e == null) {
                                                                    if (objects.size() > 0) {
                                                                        for (ParseObject events : objects) {
                                                                            if(events.getString("state").matches("done")){
